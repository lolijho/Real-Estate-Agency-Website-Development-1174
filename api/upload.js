// API Upload semplificata per risolvere il problema
export default async function handler(request, response) {
  ...
  console.log('=== API Upload chiamata ===');
  ...
  try {
    ...
    if (request.method === 'POST') {
      console.log('POST request ricevuta');
      ...
      const mockResponse = {
        ...
        success: true,
        message: 'Upload simulato con successo (mock)',
        timestamp: new Date().toISOString()
      };
      
      console.log('Restituendo mock response:', mockResponse);
      return response.status(200).json(mockResponse);
    }
    
    // Gestisci GET per elencare immagini
    else if (request.method === 'GET') {
      console.log('GET request ricevuta');
